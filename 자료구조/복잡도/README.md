...
다음 코드는 C++ 문법으로 반복 횟수를 보여준다. 반복문 안의 작업은 각각 상수 시간이라고 가정한다.
...
| 벡터 | `O(1)` | `O(n)` | `O(n)` | 재할당 없으면 `O(1)`, 있으면 `O(n)` | `O(1)` |
...
동적 배열의 크기 확장이 발생하는 추가 연산은 `O(n)`이 될 수 있다. 배열의 앞 원소를 지울 때 나머지를 매번 이동시키는 방식의 큐도 제거에 `O(n)`이 걸린다. 자료 구조의 이름과 함께 구현 방식도 확인해야 한다.
