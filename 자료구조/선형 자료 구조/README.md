...
이하의 연산 비용은 원소 하나의 비교·복사·이동을 일정한 비용으로 가정한다. 복잡도의 표기와 계산 방법은 [복잡도](../복잡도/README.md)에서 설명한다.
...
**벡터**(**Vector**)는 필요에 따라 저장 용량을 늘릴 수 있는 **동적 배열**(**Dynamic Array**)이다. 여기서는 C++의 `std::vector`를 기준으로 설명한다.
...
용량을 8로 늘리는 것은 설명을 위한 예시다. `std::vector`의 용량 증가 배율은 구현에 따라 다르며 반드시 두 배가 되는 것은 아니다. 재할당이 일어나면 기존 원소를 가리키던 포인터·참조·반복자가 무효화된다. [C++ 표준 초안의 벡터 변경 연산](https://eel.is/c++draft/vector.modifiers)
...
values.reserve(4);       // 용량을 최소 4개 확보, 크기는 0
values.push_back(10);    // 크기 1
values.push_back(20);    // 크기 2
int first = values[0];   // 인덱스로 접근
values.pop_back();       // 마지막 원소 제거, 크기 1
values.resize(3);        // 크기 3, 추가된 int 원소는 0으로 초기화
```

`reserve`는 용량을 확보하고 `resize`는 실제 원소 수를 변경한다. `reserve(4)` 직후 크기가 0인 벡터에 `values[0]`으로 접근해서는 안 된다.

| 연산 | 시간 복잡도 | 이유 |
| --- | --- | --- |
| 인덱스 접근 | `O(1)` | 연속된 메모리에서 주소를 계산한다. |
| 정렬되지 않은 값 탐색 | `O(n)` | 원소를 비교한다. |
| 뒤 삽입 | 재할당 없으면 `O(1)`, 있으면 `O(n)` | 용량 확장 시 기존 원소를 옮긴다. |
...
| `push` | 맨 위에 원소를 추가한다. |
| `pop` | 맨 위의 원소를 제거한다. |
| `top` 또는 `peek` | 맨 위 원소를 제거하지 않고 확인한다. |
| `empty` | 비어 있는지 확인한다. |

`pop`이 값을 반환하는지는 언어와 API에 따라 다르다. C++의 `std::stack::pop()`은 원소를 제거하기만 하므로 값을 사용할 때는 먼저 `top()`으로 읽는다.
...
배열에서는 마지막 사용 위치를 관리하고 연결 리스트에서는 머리에서 삽입·삭제하면 된다. 이 경우 기본 연산은 `O(1)`이다. 동적 배열로 구현하면 저장 공간을 확장하는 추가 연산에는 `O(n)`이 걸릴 수 있다.
...
| --- | --- |
| `enqueue` | 뒤에 원소를 추가한다. |
| `dequeue` | 앞에서 원소를 제거한다. |
| `front` 또는 `peek` | 다음에 제거할 원소를 확인한다. |
| `empty` | 비어 있는지 확인한다. |

C++의 `std::queue`에서는 추가에 `push`, 제거에 `pop`, 앞 원소 확인에 `front`, 마지막 원소 확인에 `back`을 사용한다. `pop()`은 값을 반환하지 않는다.
